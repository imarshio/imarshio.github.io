...
在使用安装文件而不是AppsStore安装软件时，双击后需要将待安装的软件拖到Application里，等到启动台（即平时打开软件的地方）里有这个软件的时候就可以关闭这个页面了，但是你会发现，桌面上多了一个图标，此时你需要右击这个图标，然后选择推出即可。

后续遇到其他再说。

### iCopy

剪切板，enmm，很好用，就是建议要关闭音效，开启后⌃+`C`即可呼出剪切板。
...
我目前比较喜欢使用[Typora](https://typora.io/)，但是每次打开`md`文件时，总会使用`xcode`打开，很烦。

所以我每次打开`md`文件时，都需要右击--》打开方式--》选择Typora，这样也不方便，每次打开一个新的`md`文件都要选择一下。
